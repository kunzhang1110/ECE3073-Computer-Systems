...
void interrupt_handler(void)
{
	...
	//declare variable for ipending register
	int ipending;
	...
	// PIO interrupt level 1
	if ((ipending & 0x1) == 1)
	{
	//code for counting LED same like polling program
	*( OutPort_FLAG )=1;	// connect to GPIO_1 [12]
	*( OutPort_LEDR ) = *( OutPort_LEDR ) + 1;
	*( OutPort_FLAG ) = 0;	// connect to GPIO_1 [12]

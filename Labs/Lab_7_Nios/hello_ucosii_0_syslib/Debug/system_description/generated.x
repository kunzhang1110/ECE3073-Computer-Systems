 ...
 *
 * Generated: 2013-05-15 09:39:21.821
 *
 */
...
MEMORY
{
    reset : ORIGIN = 0x00040000, LENGTH = 32
    static_ram_0 : ORIGIN = 0x00040020, LENGTH = 262112
}

    /* Define symbols for each memory base-address */
 __alt_mem_static_ram_0 = 0x00040000 ;
...
SECTIONS
{
    ...
    .text :
    {
        ...
        *(.dynamic)
        ...
        KEEP (*(.jcr))
        . = ALIGN(32 / 8);
    } >  static_ram_0 =0x3a880100 /* NOP on Nios2 (big endian) */
    ...
    .static_ram_0 LOADADDR (.bss) + SIZEOF (.bss): AT (LOADADDR (.bss) + SIZEOF (.bss))
    {
        PROVIDE (_alt_partition_static_ram_0_start = ABSOLUTE(.));
        *(.static_ram_0 .static_ram_0.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_static_ram_0_end = ABSOLUTE(.));
        _end = ABSOLUTE(.);
        end = ABSOLUTE(.);
        __alt_stack_base = ABSOLUTE(.);
    } > static_ram_0

    PROVIDE (_alt_partition_static_ram_0_load_addr = LOADADDR(.static_ram_0));
...
PROVIDE( __alt_heap_limit    = 0x00080000 );

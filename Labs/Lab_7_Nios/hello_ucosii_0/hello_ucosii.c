...
//	HEX1 - Edit Mode Indicator, C indicate clock display
//  HEX0 - Alarm Mode Indicator, A indicate alarm on

//	KEY0 - Increase displayed minutes by 1 minute per click
//  KEY1 - Increase displayed hour by 1 hour per click
//  KEY2 - Change display between clock or alarm
//	KEY3 - Change alarm mode
//*********************************************************************************************************
...
void Interrupt_changes (void* pdata)
{
    ...
    while (1)
    {
        ...
        if (status == 14) // KEY0 pressed
			OSSemPost(sem_min_increase);
            
        else if (status == 13) // KEY1 pressed   
			OSSemPost(sem_hour_increase);

        else if (status == 11) // KEY2 pressed                
            OSSemPost(sem_display_change);
			
		else if (status == 7) // KEY3 pressed
	    {
...
//*********************************************************************************************************
//                              TASK Display_change - change display
//*********************************************************************************************************
...
{
    INT8U err; // critical section required this variable
    
    while (1)
    {
      // critical section to avoid wrong addition to the value after been change by high priority tasks eg hour and mins  
      CS(  
...
void Alarm_counter (void* pdata)
{
    while (1)
    {
        if (alarm_selection != 0 && clock_hour == alarm_hour && clock_min == alarm_min)
        {
            OSSemPost(sem_alarm_light);
            OSSemAccept (sem_alarm_light); // ensure sem_alarm_light always, else LEDG light up even no alarm set
        }
        OSTimeDlyHMSM(0,0,5,0); 
    }
}
...
void LEDG_Duration (void* pdata)
{
    ...
    while (1)
    {
        ...
        OSSemPend(sem_alarm_light, 0, &err);
         while ( m < 2500) // control how long the LEDR wave oscillate about 100 per second
         {
       ...
       OSTimeDlyHMSM(0,0,5,0); // control how many oscillations within the 1 min that alarm same time as clock  
    }
}
...
{ 
    INT8U  err;
    // initialize critical section semaphore and checking
    ...
    err = OSTaskCreateExt(Hour_increase,
                        ...
                        5, // high priority as less task to do so that will not disturb task that are long
                        5,
                        task3_stk,
                        TASK_STACKSIZE,
                        NULL,
   ...
   err = OSTaskCreateExt(Interrupt_changes,
                        ...
                        8, // lower priority than those changes that occur to ensure those changes run until next pending to avoid semaphore value not equal 0
                        8,
                        task2_stk,
                        TASK_STACKSIZE,
                        NULL,
